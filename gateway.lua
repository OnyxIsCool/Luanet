...
local function hasFileSystemSupport()
    local ok1 = pcall(function() return typeof(writefile) == "function" end)
    local ok2 = pcall(function() return typeof(readfile) == "function" end)
    local ok3 = pcall(function() return typeof(isfile) == "function" end)
...
local function loadVerifiedKey()
    ...
    local success, message = auth.validate(content)
    if not success then
        return nil
    end
...
local function clearSavedKey()
    ...
    local function loadUIFactory()
        ...
        return function(self)
            ...
            self.gui.Name = "GengarKeySystemUI"
            ...
            brandLogoText.Text = "Gengar Key System"
            ...
            self.gui.AncestryChanged:Connect(function(_, parent)
                if parent == nil then
                    local blur = Lighting:FindFirstChild("GengarUIBlur")
    ...
    function UI.new(options)
        ...
        self.title = self.options.title or "Gengar Authentication System"
        self.subtitle = self.options.subtitle or "Powered by PandaDevelopment"
...
function UI:handleGetLink()
    local secureGetKeyLink = auth.copyKeyUrl()
...
function UI:handleVerifyKey()
    ...
    local result, message = auth.validate(key)
...
local result, message = auth.validate(keyToCheck)
if result then
...
if savedKey and not result then
    clearSavedKey()
end
