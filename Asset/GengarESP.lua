...
-- credits to mstudio45 for the original esp code

--[[
     https://discord.gg/q5BFSBW4Z
...
local VERSION = "2.0.4";
local DEBUG_ENABLED = getgenv().gengaresp_ESP_DEBUG == true;

local debug_print = if DEBUG_ENABLED then (function(...) print("[Gengar ESP]", ...) end) else (function() end);
local debug_warn  = if DEBUG_ENABLED then (function(...) warn("[Gengar ESP]", ...) end) else (function() end);
-- local debug_error = if DEBUG_ENABLED then (function(...) error("[Gengar ESP] " .. table.concat({ ... }, " ")) end) else (function() end);

if getgenv().gengaresp_ESP then
	debug_warn("Already Loaded.")
	return getgenv().gengaresp_ESP
end
